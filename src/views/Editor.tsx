...
export default function Editor({ onNavigate }: EditorProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="pb-32 px-6 max-w-7xl mx-auto"
    >
      {/* Editor Toolbar */}
      <div className="sticky top-20 z-40 bg-primary-900/60 backdrop-blur-xl border-b border-white/5 -mx-6 px-6 py-4 mb-8">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
          ...
          <div className="ml-auto flex gap-2">
            <button 
              ...
              className={`px-8 py-2.5 bg-ai-blue text-white rounded-full font-bold text-sm ai-glow active:scale-95 transition-all flex items-center gap-2 ${isExporting ? 'opacity-50' : ''}`}
            >
              {isExporting ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        ...
        <div className="md:col-span-8 flex flex-col gap-6">
          <div className="bg-[#1C1C1E] border border-white/5 rounded-[40px] p-8 md:p-14 flex items-center justify-center min-h-[600px] relative overflow-hidden group shadow-2xl">
            ...
            <div className="absolute top-8 left-1/2 -translate-x-1/2 z-20 flex items-center gap-4 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
               <button 
               ...
               <span className="text-xs font-bold text-white uppercase tracking-widest min-w-[80px] text-center">
            ...
            <motion.div 
              ...
              className="bg-white w-full max-w-[420px] aspect-[1/1.41] shadow-[0_40px_70px_rgba(0,0,0,0.4)] rounded-sm overflow-hidden relative"
            >
              <div className="p-10 space-y-6 opacity-80 select-none">
                <div className="flex justify-between items-start">
                   <div className="h-8 w-1/3 bg-gray-200 rounded"></div>
                   <div className="w-10 h-10 rounded-full bg-ai-blue/5 border border-ai-blue/10 flex items-center justify-center text-[10px] font-bold text-ai-blue">
                ...
                {currentPageIndex === 0 && (
                  <div className="pt-16 space-y-4">
                     <div className="h-32 w-full bg-primary-50 rounded-xl border-2 border-primary-100 border-dashed flex items-center justify-center relative group/sign">
                       {signed ? (
                         <motion.div initial={{ scale: 0, rotate: -5 }} animate={{ scale: 1, rotate: -2 }} className="absolute">
                           <svg width="180" height="60" viewBox="0 0 120 40" className="text-ai-blue drop-shadow-sm">
                             <path d="M10 30 Q30 10 50 25 T90 15" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
                           </svg>
                         </motion.div>
                       ) : (
                         <div className="text-center">
                           <PenTool className="w-6 h-6 text-primary-200 mx-auto mb-2" />
                           <span className="text-[10px] text-gray-300 font-bold uppercase tracking-widest">Sign required</span>
                         </div>
                       )}
                     </div>
                  </div>
        ...
        <div className="md:col-span-4 space-y-6">
          ...
          <div className="glass-card rounded-[32px] p-8 border border-white/10 bg-gradient-to-b from-white/[0.05] to-transparent">
             ...
             <div className="space-y-4">
                <MetaItem label="File Name" value="Invoice_ZenScan_04.pdf" />
                <MetaItem label="Created" value="May 09, 2026" />
                <MetaItem label="Pages" value={`${pages.length} Pages`} />
                <MetaItem label="Size" value={`${(pages.length * 0.4).toFixed(1)} MB`} />
                <MetaItem label="Type" value="Invoice" />
             </div>
